...
public class AnnotationReader extends ClassDataCollector {
	...
	public AnnotationReader(Analyzer analyzer, Clazz clazz) {
	...
	protected void doReference(Reference reference, Annotation raw) throws Exception {
		...
		if (component.references.containsKey(def.name))
			...
			component.references.put(def.name, def);
                ...
                char type = hackType(reference);
                if(type == '*') {
                    def.cardinality = ReferenceCardinality.MULTIPLE;
                } else if(type == '?') {
                    def.cardinality = ReferenceCardinality.OPTIONAL;
                } else if(type == '+') {
    ...
    private char hackType(Reference reference) {
        try {
            return reference.type();
        } catch(IllegalArgumentException x) {
            final Matcher matcher = Pattern.compile("No conversion to class java.lang.Character from class java.lang.* value (.*)").matcher(x.getMessage());
            if(matcher.matches()) {
                return (char)Integer.parseInt(matcher.group(1));
            }
            throw x;
        }
    }
